...
# ---------------------------------------------------------------------------
# SMI-5922 — private-registry-get auth-gate canary.
# ---------------------------------------------------------------------------

_require_private_registry_get_supabase_url() {
  if [ -z "${SUPABASE_URL:-}" ]; then
    smoke_warn "SUPABASE_URL not set — failing private-registry-get check"
    return 1
  fi
  return 0
}

# _private_registry_get_probe URL
# GET request, no auth. Echoes the HTTP status on line 1, then the raw
# response headers. Body is discarded. "000" status on connection failure
# (so with_retry can retry). Mirrors api-proxy.sh's _api_proxy_probe idiom.
_private_registry_get_probe() {
  local url="$1"
  local tmp code
  tmp=$(mktemp)
...
# ---- check_private_registry_get_requires_jwt -----------------------------
# GET (no auth) with a valid-shaped skillId. private-registry-get/index.ts
# rejects any non-GET method with 405 BEFORE authenticateUser() ever runs
# (SMI-5922 plan review finding #1), so a POST probe would fail on the wrong
# status code and prove nothing about the auth gate -- this must be GET.
# validateParams() also runs before auth, so skillId must be well-formed
# (author/name) or the response would be a 400 param-validation failure
# instead of the 401 we're proving. Expect exactly 401 + Cache-Control:
# no-store (access.ts sets this literal value on every exit path). 000/5xx/
# 200/404/405 are all treated as fail -- mirrors
# check_inventory_upload_requires_jwt's "transient blip must not mask a
# regression" rule (inventory.sh).
check_private_registry_get_requires_jwt() {
  _require_private_registry_get_supabase_url || {
    report_fail "edge-fn-private-registry-get" "check_private_registry_get_requires_jwt" "" "SUPABASE_URL" "unset"
    return 1
  }
  local url="${SUPABASE_URL}/functions/v1/private-registry-get?skillId=acme/test-skill"
  local t0 t1 ms out status headers
  t0=$(now_ms)
  out=$(with_retry _private_registry_get_probe "$url")
  t1=$(now_ms)
  ms=$((t1 - t0))
  status=$(printf '%s' "$out" | head -n1)
  headers=$(printf '%s' "$out" | tail -n +2 | tr -d '\r')

  if [ "$status" != "401" ]; then
    report_fail "edge-fn-private-registry-get" "check_private_registry_get_requires_jwt" "$url" "401" "$status" "$ms"
    return 1
  fi

  if ! printf '%s' "$headers" | grep -qi '^cache-control: *no-store$'; then
    report_fail "edge-fn-private-registry-get" "check_private_registry_get_requires_jwt" "$url" "401 + Cache-Control: no-store" "401 (Cache-Control: no-store missing)" "$ms"
    return 1
  fi
