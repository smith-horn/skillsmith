...
function validateNamingConvention(filePath, conventions) {
  ...
  return conventions.files.components.test(fileName) || conventions.files.utilities.test(fileName)
}
...
async function main() {
  ...
  if (fileIdx !== -1 && args[fileIdx + 1]) {
    ...
    if (/^[A-Z][a-zA-Z0-9]*$/.test(moduleName)) {
      ...
      fail(
        `Module name "${moduleName}" should be PascalCase`,
        `Use: ${moduleName.charAt(0).toUpperCase() + moduleName.slice(1)}`
      )
    }
  } else if (runAll) {
  ...
  for (const filePath of targetFiles) {
    ...
    if (validateNamingConvention(filePath, conventions)) {
      ...
      fail(
        'File name does not follow naming conventions',
        'See standards.md section 1.2 for conventions'
      )
    }
    ...
    if (checkTypesExist(filePath)) {
      ...
      warn(
        ...
        'Create types before implementation or define interfaces in file'
      )
    }
    ...
    if (jsdocCheck.hasJSDoc) {
      if (jsdocCheck.functions > 0) {
        pass(
          `JSDoc present (${jsdocCheck.documented}/${jsdocCheck.functions} functions documented)`
        )
      } else {
      ...
      warn(
        `Functions lack JSDoc (${jsdocCheck.documented}/${jsdocCheck.functions} documented)`,
