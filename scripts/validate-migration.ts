...
async function validate() {
  ...
  console.log(
    `Sampling ${sampleSize} random skills (${(SAMPLE_PERCENTAGE * 100).toFixed(1)}% of ${sqliteCount.count})...`
  )
  ...
  if (samplesError) {
    ...
    for (const id of sampleIds) {
      const sqliteSkill = sqlite.prepare('SELECT * FROM skills WHERE id = ?').get(id) as SQLiteSkill
  ...
  if (integrityError || !supabaseSample) {
    ...
    const checks = [
      ...
      { field: 'author', match: sqliteSample.author === supabaseSample.author },
      {
        field: 'quality_score',
        match:
          Math.abs((sqliteSample.quality_score || 0) - (supabaseSample.quality_score || 0)) < 0.001,
      },
  ...
  if (passed) {
    ...
    if (typeof matchCount === 'number' && sampleSize > 0) {
      console.log(
        `Sample integrity: ${matchCount}/${sampleSize} (${((matchCount / sampleSize) * 100).toFixed(1)}%)`
      )
    }
    console.log('\nNext steps:')
