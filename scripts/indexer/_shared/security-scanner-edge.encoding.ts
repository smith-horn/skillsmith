...
// fix — the pattern is no longer invisible: the first
// MAX_OVERSIZED_ADVISORIES already surface it. Reset fresh on every
// scanEncodedPayload call.
...
export function scanEncodedPayload(
  ...
  for (const [index, line] of lines.entries()) {
    ...
    for (const match of line.matchAll(BASE64_CANDIDATE)) {
      ...
      // MAX_ENCODED_CANDIDATE_BYTES's comment for the evasion this closes.
      // Emitted at low/low: it is a coverage caveat, not evidence.
      if (candidate.length > MAX_ENCODED_CANDIDATE_BYTES) {
        if (oversizedAdvisories >= MAX_OVERSIZED_ADVISORIES) continue
        oversizedAdvisories++
        const { inDocContext: oversizeInDocContext } = classifyMatch(contexts[index], line, start)
        findings.push({
