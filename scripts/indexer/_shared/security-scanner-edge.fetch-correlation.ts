/**
 * SMI-6033 Wave 2: Edge shared fetch-correlation utilities
 * @module scripts/indexer/_shared/security-scanner-edge.fetch-correlation (Node port)
 *
 * Extracted out of security-scanner-edge.compound.ts so every
 ...
 * their arguments — no scanner instance state. Byte-identical body across
 * both _shared twins (parity test enforces); only the @module header line
 * above differs. Mirrors the core reference implementation
 * (SecurityScanner.fetch-correlation.ts) — pure extraction, no behavior
 * change: the exported names/bodies are unchanged from their previous inline
 * copies in security-scanner-edge.compound.ts (only `CHMOD_FETCH_CONTEXT` is
 * renamed to the generic `FETCH_COMMAND_PATTERN`, since it is no longer
 * chmod-specific).
 */
