/**
 * SMI-5207: Edge MF-3 path action-context gate
 ...
 * gate, symmetric with security-scanner-edge.value-gate.ts (the MF-4 half).
 * Mirrors core's identical SecurityScanner.action-context.ts split. Cohesive
 * unit — every symbol here serves the single question "is this path being
 * acted on?". Byte-identical body across both `_shared` twins (parity test
 * enforces); only the @module header line above differs. Pure Deno/Web APIs,
 * no Node deps.
 */
 ...
 * (its category cap limits its contribution to 4.00/100 against the 40-point
 * indexer quarantine threshold).
 *
 ...
 * in security-scanner-edge.prose-lexicon.ts (line-budget split only). A
 * negated verb is not action evidence — "This guide downloads no files and
 * explains ~/.ssh/config" (round-3 counter-example, unquoted prose) must not
 * fire HIGH. The negation token can appear BEFORE ("never uploads") or AFTER
 * ("downloads no files") the verb, so the window is bidirectional.
 * Deliberately narrow: same line only, small window, no general negation
 * engine.
 */
 ...
 * 4.00/100 on both indexer substrates and cannot quarantine alone there. (2)
 * Corpus evidence: none of the 13 live entries in
 ...
 * an over-flag on the one allowlisted surface (weekly scan) is recoverable
 * exactly as all 10 prior recurrences were, whereas the alternative — a
