...
// CodeQL js/clear-text-logging flagged this sink: `detail` sometimes derives from a property on
// a fetch response from an auth-adjacent endpoint, and CodeQL's source heuristic treats any
// property read off such a response as "sensitive," regardless of which property. Verified false
// positive -- every `detail` here is a plain HTTP status code, a Cache-Control value, or a
// denial-response error body (e.g. `{"error":"Authentication required"}`); rawPrivateRegistryGet()
// is only ever called on the 401/403/404 denial paths, never with real skill content, so no
// credential/PII can reach this log line.
function record(row: string, pass: boolean, detail?: string): void {
  results.push({ row, pass, detail })
  const status = pass ? 'PASS' : 'FAIL'
  console.error(`[e2e-registry] ${status} ${row}${detail ? ` — ${detail}` : ''}`) // codeql[js/clear-text-logging]
}
...
async function main(): Promise<void> {
  ...
  if (failed.length > 0) {
    ...
    for (const f of failed) {
      // Same verified false positive as record()'s suppression above.
      console.error(`  - ${f.row}: ${f.detail ?? '(no detail)'}`) // codeql[js/clear-text-logging]
    }
