#!/usr/bin/env bash
# SMI-6208: detects a `Deploy to Production` job (website-deploy-staging.yml)
# that was cancelled while `waiting` for required-reviewer approval or while
# actively mid-deploy -- the exact incident class this issue was filed for
# (PR #2546/SMI-6194's production job was silently cancelled by a same-day
# unrelated push, 2026-08-27, before this fix). The concurrency-scoping fix
# in website-deploy-staging.yml (job-level `website-production-deploy-*`
# group, cancel-in-progress: false) is the real root-cause fix -- this
# script is a backstop in case some other path still cancels that job.
#
...
while IFS=$'\t' read -r RUN_ID HEAD_SHA CREATED_AT RUN_URL; do
  ...
  # --- 3. Did THIS run's "Deploy to Production" job get cancelled after
  #        reaching the approval gate (not before, via ordinary supersede)? ---
...
3. If this recurs, check whether \`website-deploy-staging.yml\`'s
   \`deploy-production\` job-level concurrency group
   (\`website-production-deploy-*\`, \`cancel-in-progress: false\`) was
   accidentally reverted or weakened -- that mechanism is what should
   prevent this class of cancellation. See
