...
# SMI-5708 Item #9: the same real-mode run that updates baseline.json also
# appends a matching entry to .signatures.log (eval-runner-signatures.ts's
# emitBaselineSignature). Staged alongside baseline.json below so the
# committed baseline is never left without a signature the pre-push
# validator can find for the next human touching ranking files.
REL_SIGNATURES="packages/doc-retrieval-mcp/eval/.signatures.log"
...
# SMI-5708 Item #9 (Codex review finding): emitBaselineSignature() failures
# are deliberately non-fatal to the real-mode run itself (an I/O hiccup
# should not invalidate an otherwise-good baseline write) -- but that design
# was chosen for the INTERACTIVE developer case, where a WARNING in the
# run's own stdout is something a human might notice before pushing. This
# cron runs unattended; nothing reads its stdout in real time. Without this
# check, a rare signature-emission failure here would silently commit a
# drifted baseline.json with no matching entry in .signatures.log --
# reproducing the exact bug this item exists to close, with no link back to
# this run visible to whichever developer's later, unrelated push trips the
# pre-push validator weeks from now. Abort the auto-PR (not the whole run --
# the heartbeat is already written) if the post-eval baseline's hash has no
# matching entry in the signature log.
if ! cut -f1 "$EVAL_CLONE/$REL_SIGNATURES" 2>/dev/null | grep -qx "$POST_HASH"; then
  log "ERROR: baseline.json changed but no matching entry for $POST_HASH found in $REL_SIGNATURES -- signature emission likely failed this run. Skipping auto-PR to avoid landing an unsigned baseline; will retry next run."
...
PR_OK=1
{
  ...
  git -C "$EVAL_CLONE" add "$REL_BASELINE" "$REL_HEARTBEAT" "$REL_SIGNATURES" &&
