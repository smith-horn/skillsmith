...
# SMI-4689: generate_docker_override moved to scripts/_lib.sh so
# repair-worktrees.sh can reuse it for the regen step. Behavior is
# byte-equivalent on Linux; on macOS it now also emits per-package
# node_modules bind mounts.
...
_print_probe_timeout_warning() {
    ...
    echo "    (b) The container's node_modules named volume is not built yet. Build it"
    echo "        inside the container, only while that volume is really attached (a"
    echo "        detached volume makes npm write the host's macOS tree, SMI-6516):"
    echo "        docker exec $SHARED_CONTAINER sh -c 'mountpoint -q /app/node_modules && npm install && npm run build'"
    echo "        If it exits 1 with no npm output, the volume is detached: recreate the"
    echo "        container from the main checkout (docker compose --profile dev up -d --force-recreate dev)."
    echo ""
