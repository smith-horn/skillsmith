...
describe('create-worktree.sh Step 8 readiness probe (SMI-5596)', () => {
  ...
  it('T1: prints the actionable boxed warning (both causes named) and still exits 0 on timeout', () => {
    ...
    expect(result.output).toMatch(/node_modules named volumes[\s\S]*is not built/)
    // SMI-6614 (round-2b): the install remedy runs only behind the shared
    // node-modules-mount-gate.sh (checks every node_modules named volume,
    // not just root), and the build runs in the container, never on the
    // host after `docker exec` exits.
    expect(result.output).toContain(
      "sh -c 'sh scripts/lib/node-modules-mount-gate.sh && npm install && npm run build'"
    )
