 ...
 *
 * SMI-6680 (post-merge governance retro on PR #2855) split the implementation out of the original
 * ~1000-line test file into four siblings so every non-test file here stays under the repo's
 * 500-line gate -- `private-registry-audit-trigger.scanner.ts` (comment/quote-aware text scanning),
 ...
 * "later migration" tripwires). A later gate round on PR #2860 split the original test file itself
 * (`.static.test.ts`, by then also over the file-length gate) along that same implementation seam:
 * THIS file holds the migration pins and semantic checks below, plus `pinRemediation()`'s own
 * coverage; `private-registry-audit-trigger.detectors.test.ts` holds the fixture-driven exercise of
 * the five detectors (its own `describe('later-migration tripwires ...')` block drives all five
 * against `mkdtempSync()` fixtures -- SMI-6680's own finding F1: every detector iterated
 * `laterMigrationFiles()` against the REAL migrations directory, which returns `[]` (this migration
 * is the newest of 207 files today), so none of the five detectors was ever exercised by a
 * committed test until that fixture suite existed); and
 * `private-registry-audit-trigger.scanner.test.ts` holds the `splitStatements()`/`stripComments()`
 * case tables. `.static.test.ts` itself is gone -- every line of it mapped cleanly onto one of
 * these three files, with no leftover content that belonged to none of them.
 *
...
import { stripLineComments } from './private-registry-audit-trigger.scanner.ts'
import {
