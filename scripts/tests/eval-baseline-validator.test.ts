 ...
 *
 * The validator inspects the *actual* repo at scripts/eval-baseline-validator.mjs;
 * to avoid mutating the real repo's baseline.json or .signatures.log, we run
 * each scenario in an isolated temp git repo that replicates the path layout
 * (packages/doc-retrieval-mcp/eval/{baseline.json,.signatures.log}, scripts/),
 * copy the validator script in, and exec it from there.
 */
...
function git(cwd: string, ...args: string[]): string {
  // SMI-4693: route every git invocation through makeFixtureEnv() so
...
function setupRepo(opts: {
  ...
  // We synthesize a push from headSha; remote_sha=baseSha so the diff range
  // is baseSha..headSha (matches the file changes above).
  void headSha
  void baseSha
...
function run(
  ...
  // SMI-4693: spawnSync also needs makeFixtureEnv() — the validator itself
  // shells out to git inside the fixture repo.
