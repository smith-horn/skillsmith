...
# round-3: this scenario invokes $GUARD directly (bypassing run_guard()'s
# wrapper, since it needs two independently-timed invocations sharing one
# APP7) — so, unlike every run_guard()-mediated scenario above, it must
# forward APP_ROOT/NODE_MODULES_MOUNT_GATE_MOUNTINFO itself. Without this,
# node-modules-mount-gate.sh falls through to its real defaults (/app,
# /proc/self/mountinfo) and reports THIS CONTAINER's actual mount state
# instead of $APP7's fixture — irrelevant to the concurrency behavior this
# scenario exists to test, and (inside a worktree container, per this
# round's own finding) usually a real detached mount, which would make the
# guard refuse before ever reaching the lock/install logic under test.
( cd "$MAIN7A" && FAKE_APP_DIR="$APP7" APP_ROOT="$APP7" \
    NODE_MODULES_MOUNT_GATE_MOUNTINFO="$(fake_mountinfo_path "$APP7")" \
...
( cd "$MAIN7B" && FAKE_APP_DIR="$APP7" APP_ROOT="$APP7" \
    NODE_MODULES_MOUNT_GATE_MOUNTINFO="$(fake_mountinfo_path "$APP7")" \
...
# round-3: direct $GUARD invocation (see Scenario 7's comment above) — must
# forward APP_ROOT/NODE_MODULES_MOUNT_GATE_MOUNTINFO itself.
( cd "$MAIN8A" && FAKE_APP_DIR="$APP8" APP_ROOT="$APP8" \
    NODE_MODULES_MOUNT_GATE_MOUNTINFO="$(fake_mountinfo_path "$APP8")" \
    FAKE_DOCKER_LOG="$(mktemp)" SKILLSMITH_NATIVE_CHECK_TEST=ok "$GUARD" </dev/null; echo $? > "$LOG_8A" ) &
PID_8A=$!
( cd "$MAIN8B" && FAKE_APP_DIR="$APP8" APP_ROOT="$APP8" \
    NODE_MODULES_MOUNT_GATE_MOUNTINFO="$(fake_mountinfo_path "$APP8")" \
