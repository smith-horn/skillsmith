...
function generateMarkdownReport(report: CombinedReport): string {
  ...
  lines.push('## Summary')
  lines.push('')
  lines.push(`- **Status**: ${report.overall.passed ? '✅ PASSED' : '❌ FAILED'}`)
  ...
  if (report.cli) {
    const status = report.cli.passed ? '✅ Pass' : '❌ Fail'
    lines.push(`| CLI E2E | ${status} | ${(report.cli.duration / 1000).toFixed(2)}s |`)
  } else {
    lines.push('| CLI E2E | ⏭️ Skipped | - |')
  }

  if (report.mcp) {
    const status = report.mcp.passed ? '✅ Pass' : '❌ Fail'
    lines.push(`| MCP E2E | ${status} | ${(report.mcp.duration / 1000).toFixed(2)}s |`)
  } else {
    lines.push('| MCP E2E | ⏭️ Skipped | - |')
  }
...
async function main(): Promise<void> {
  ...
  if (hardcodedIssues.length > 0) {
    console.log(`\n⚠️  ${hardcodedIssues.length} hardcoded issue(s) detected!`)
  }
