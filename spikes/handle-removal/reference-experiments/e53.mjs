...
if (!/exp8|^\/tmp\/e53|^\/work|^\/exp\/e53/.test(BASE)) throw new Error('unsafe base')
...
for (const [variant, race, stub] of cells) {
  ...
  for (let t = 0; t < n; t++) {
    ...
    fs.writeFileSync(path.join(top, 'z.txt'), 'z\n')
    fs.mkdirSync(sub)
    ...
    try {
      ...
      r[e.code]++
      if (!race && fs.existsSync(path.join(sub, 'a.txt')) && fs.existsSync(path.join(top, 'z.txt')))
