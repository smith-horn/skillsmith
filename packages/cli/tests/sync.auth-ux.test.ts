...
const mocks = vi.hoisted(() => {
  // SMI-6236 F-2: real Error subclasses (not just vi.fn()) so
  // requireSyncTier()'s `instanceof` branching works the same way it does
  // against the real @skillsmith/core exports. Defined inside vi.hoisted so
  // they're guaranteed to exist before any vi.mock() factory below runs.
  class SessionTierAuthError extends Error {
    constructor(message = 'Not authenticated. Run `skillsmith login` and try again.') {
      super(message)
      this.name = 'SessionTierAuthError'
    }
  }
  class SessionTierTransientError extends Error {
...
vi.mock('@skillsmith/core', () => ({
  ...
  // SMI-6236 F-2: requireSyncTier()'s three-way credential resolution.
  resolveSessionTier: (...args: unknown[]) => mocks.resolveSessionTier(...args),
  SessionTierAuthError: mocks.SessionTierAuthError,
  SessionTierTransientError: mocks.SessionTierTransientError,
}))
...
//
// SMI-6236 F-2: resolveSessionTier defaults to "no stored JWT session"
// (SessionTierAuthError) and SKILLSMITH_API_KEY defaults to unset, so
// requireSyncTier() falls through to requireTier('team') by default —
// preserving every pre-existing test's behavior unchanged. Tests that
// specifically exercise the JWT/API-key branches override these directly.
beforeEach(() => {
  mocks.requireTier.mockResolvedValue(undefined)
  mocks.resolveSessionTier.mockRejectedValue(new mocks.SessionTierAuthError())
...
// ---------------------------------------------------------------------------
// SMI-6236 F-2 (security review): requireSyncTier()'s three-way credential
// resolution. requireTier('team') alone resolves tier from
// SKILLSMITH_LICENSE_KEY only — completely disconnected from the credential
// a real sync request authenticates with (a stored JWT session, or
// SKILLSMITH_API_KEY) — so a JWT-only Team customer (the documented
// `skillsmith login` flow) was being incorrectly blocked as community before
// ever reaching the server. These tests prove the fix resolves tier via
// whichever credential the request will actually use.
// ---------------------------------------------------------------------------

describe('SMI-6236 F-2: requireSyncTier() credential resolution', () => {
  let exitSpy: ReturnType<typeof vi.spyOn>
  ...
  it('a JWT session resolving to Team tier is admitted without ever calling requireTier', async () => {
    mocks.resolveSessionTier.mockResolvedValue({ authenticated: true, tier: 'team' })
    mocks.requireTier.mockRejectedValue(
      new Error('requireTier must not be called when a JWT session resolves the tier')
    )
    mocks.runRegistrySync.mockResolvedValue(
      makeResult({ success: true, totalProcessed: 1, skillsAdded: 1 })
    )
  ...
  it('a JWT session resolving below Team tier is blocked with the upgrade message, without calling requireTier', async () => {
    mocks.resolveSessionTier.mockResolvedValue({ authenticated: true, tier: 'individual' })
    mocks.requireTier.mockRejectedValue(
      new Error('requireTier must not be called when a JWT session resolves the tier')
    )
  ...
  it('a JWT session resolving authenticated:false is blocked, not silently admitted', async () => {
    mocks.resolveSessionTier.mockResolvedValue({ authenticated: false })
  ...
  it('a transient session-tier-check failure does NOT block — falls through to the sync running (server backstop)', async () => {
    mocks.resolveSessionTier.mockRejectedValue(
      new mocks.SessionTierTransientError('license-status returned HTTP 503')
    )
    mocks.requireTier.mockRejectedValue(
      new Error('requireTier must not be called on a transient session-tier failure')
    )
    mocks.runRegistrySync.mockResolvedValue(
      makeResult({ success: true, totalProcessed: 1, skillsAdded: 1 })
    )
    ...
    // Must not have been downgraded to community just because the tier
    // check itself glitched — the sync proceeds, server-side gate decides.
    expect(mocks.runRegistrySync).toHaveBeenCalled()
  })

  it('no JWT session but SKILLSMITH_API_KEY set: skips the offline check and lets the server 403 surface as the upgrade message', async () => {
    process.env['SKILLSMITH_API_KEY'] = 'sk_live_test_below_tier_key'
    mocks.requireTier.mockRejectedValue(
      new Error('requireTier must not be called when SKILLSMITH_API_KEY is set')
    )
    mocks.runRegistrySync.mockResolvedValue(
      makeResult({
        success: false,
        errors: [
          'Fetch error at offset 0: Registry sync requires a Team or Enterprise subscription',
        ],
      })
    )

    const { createSyncCommand } = await import('../src/commands/sync.js')
    // isTierRejection's path sets process.exitCode = 1 and returns normally
    // (mirrors isAuthFailure's existing SMI-4482 pattern) — it does not call
    // process.exit(), so parseAsync resolves rather than rejecting.
    await createSyncCommand().parseAsync(['node', 'test', '--yes'])

    expect(mocks.runRegistrySync).toHaveBeenCalled()
    expect(process.exitCode).toBe(1)
    const stderr = mockConsoleError.mock.calls.map((c) => String(c[0])).join('\n')
    expect(stderr).toContain('Team or Enterprise subscription')
  })

  it('no JWT session, no SKILLSMITH_API_KEY: falls back to the offline requireTier(team) check', async () => {
    mocks.requireTier.mockRejectedValue(
      new Error(
        'This feature requires team tier or higher ($25/user/month). ' +
          'You are currently on the community tier.'
