/**
 * SMI-3484: CLI Install Command — action implementation.
 *
 * SMI-5127+ sibling-split convention: action impls + withTelemetry-wrapped
 * exports live here; `install.ts` keeps only the commander factory. Split out
 * once ADR-139's (SMI-6274 Wave 4) `--scope` resolution pushed `install.ts`
 * past the 500-line standard.
 */
...
async function installActionImpl(
  ...
  try {
    ...
    // ADR-139 (SMI-6274 Wave 4): resolve global-vs-workspace scope via the
    // shared core resolver. An unsatisfiable explicit `--scope workspace`
    // throws UnsatisfiableWorkspaceScopeError, which falls through to this
    // function's existing outer catch (same handling as any other
    // pre-flight validation error below) — a hard error, never a silent
    // downgrade to global (ADR-139 point 2).
    const scopeTarget = resolveScopedSkillsDir({
      client,
      explicitScope: parseInstallScope(opts.scope),
      globalManifestPath: DEFAULT_MANIFEST_PATH,
    })
    const skillsDir = scopeTarget.dir

    if (scopeTarget.created && !jsonOutput) {
      console.log(chalk.dim(`  Created workspace skills directory: ${scopeTarget.dir}`))
    }
