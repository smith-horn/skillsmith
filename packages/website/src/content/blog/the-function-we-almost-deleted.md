...
This first pass was small on purpose. Ninety-eight lines out of a codebase with a couple thousand test files isn't going to show up in a build-time chart, and pretending otherwise would be the kind of overclaiming we'd rather not do. What it *does* prove is that the safety net holds under a real test, not just a hypothetical one. The tool caught genuinely dead code. It also almost shipped two false positives, and the verification pass caught both before they became a real regression. That's the whole point of building this as a "propose, then verify, then act" pipeline instead of a "find and delete" script: the interesting failures don't show up until you actually run it against a real, messy codebase with real conventions the tool has never seen before.

The Docker fix is still ahead of us, tracked as its own piece of work with its own review. The dead-code tool keeps running, one package at a time, each pass adding to what it's learned about our own codebase's quirks. Neither one is a silver bullet by itself. Together, they're a more honest answer than "the codebase got bigger" ever was.
