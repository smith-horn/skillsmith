...
export class ApprovalRepository {
  ...
  getApprovals(skillId: string): ApprovalEntry[] {
    const rows = this.db
      .prepare(SELECT_BY_SKILL_ID_QUERY)
      .all(skillId) as ApprovalRow[]
  ...
  getPendingApprovals(skillId: string): ApprovalEntry[] {
    const rows = this.db
      .prepare(SELECT_PENDING_BY_SKILL_ID_QUERY)
      .all(skillId) as ApprovalRow[]
  ...
  hasReviewerApproved(skillId: string, reviewerId: string): boolean {
    const row = this.db
      .prepare(CHECK_REVIEWER_EXISTS_QUERY)
      .get(skillId, reviewerId)
  ...
  isComplete(skillId: string, requiredApprovals: number = 2): boolean {
    const { count } = this.db
      .prepare(COUNT_PENDING_APPROVALS_QUERY)
  ...
  clearApprovals(skillId: string): number {
    const result = this.db
      .prepare(DELETE_BY_SKILL_ID_QUERY)
      .run(skillId)
  ...
  getWorkflowStartTime(skillId: string): string | null {
    const row = this.db
      .prepare(SELECT_FIRST_PENDING_QUERY)
      .get(skillId) as { created_at: string } | undefined
