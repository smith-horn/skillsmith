/**
 * Security Scanner — prose lexicons for the SMI-5207 sensitive_path gates
 * @module @skillsmith/core/security/scanner/SecurityScanner.prose-lexicon
 *
 * SMI-5207: NEGATION_TOKENS (MF-3) and PROSE_STOPWORDS (MF-4) split out of
 * SecurityScanner.scanners.ts purely for the 500-line pre-commit gate — the
 * two word lists are ~120 lines once Prettier expands them one entry per
 * line, which would push that file past the limit. No logic lives here; both
 * gates' behaviour is entirely in SecurityScanner.scanners.ts.
 *
 * The two lists have no consumer in common (round 6 deleted MF-3's
 * blockquote-vs-operator content classification, which was the only thing
 * that ever made MF-3 read PROSE_STOPWORDS) — they share a file for the line
 * budget, not because they are one concept. Keep them independently editable.
 */
