...
export function ownedLogPattern(surfaces: readonly string[]): RegExp {
  // Own nothing rather than something: an empty list -- or one whose names are all
  // empty -- degenerates the alternation to `()` / `(mcp|)`, both of which own
  // `skillsmith--<date>.jsonl`. `(?!)` is a negative lookahead on the empty pattern,
  // so it can never match; `new RegExp('')` would match everything (PR #2921
  // post-merge retro F6, and the governance round on its fix: an empty name is the
  // same defect one identifier over, so the guard sits after the filter).
