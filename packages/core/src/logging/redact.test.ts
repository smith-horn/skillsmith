...
-----END PRIVATE KEY-----`
  ...
  describe('redactSensitiveObject', () => {
    ...
    it('should mark a directly circular reference as [Circular] instead of stack-overflowing (SMI-5615 NEW-2)', () => {
      const circular: Record<string, unknown> = { name: 'ghp_1234567890abcdefghijklmnopqrstuvwxyz' }
      circular.self = circular
      const result = redactSensitiveObject(circular) as Record<string, unknown>
      expect(result.name).toBe('ghp_[REDACTED]')
      expect(result.self).toBe('[Circular]')
    })

    it('should mark an indirect (two-hop) circular reference as [Circular]', () => {
      const a: Record<string, unknown> = {}
      const b: Record<string, unknown> = { a }
      a.b = b
      const result = redactSensitiveObject(a) as Record<string, unknown>
      const nestedB = result.b as Record<string, unknown>
      expect(nestedB.a).toBe('[Circular]')
    })

    it('should NOT treat the same object appearing twice at sibling (non-cyclic) positions as circular', () => {
      const shared = { key: 'ghp_1234567890abcdefghijklmnopqrstuvwxyz' }
      const input = { first: shared, second: shared }
      const result = redactSensitiveObject(input) as Record<string, unknown>
      expect((result.first as Record<string, unknown>).key).toBe('ghp_[REDACTED]')
      expect((result.second as Record<string, unknown>).key).toBe('ghp_[REDACTED]')
    })
  })

  describe('stripAnsi (SMI-5615 Mode-B Wave 2 pass)', () => {
    it('should strip chalk-style SGR color codes', () => {
      const chalkRed = '\x1b[31mError:\x1b[39m something failed'
      expect(stripAnsi(chalkRed)).toBe('Error: something failed')
    })

    it('should leave plain text with no escape codes untouched', () => {
      expect(stripAnsi('plain text, no colors')).toBe('plain text, no colors')
    })

    it('should handle empty and null-ish input like redactSensitiveData does', () => {
      expect(stripAnsi('')).toBe('')
    })

    it('closes the leading word-boundary redaction gap: a secret immediately after an SGR code is still redacted once ANSI is stripped first', () => {
      // \x1b[31m ends in 'm' (a word char) directly followed by the secret's
      // 's' (also a word char) — no \b between them, so redactSensitiveData
      // alone would miss this. stripAnsi-then-redact is the fix (see
      // `redactForDisk` in logger.ts).
      const secret = 'sk_live_' + 'c'.repeat(24)
      const chalkWrapped = `\x1b[31m${secret}\x1b[39m`

      // Demonstrates the gap exists when redacting the raw (un-stripped) text:
      expect(redactSensitiveData(chalkWrapped)).toContain(secret)

      // ...and that stripping ANSI first closes it:
      expect(redactSensitiveData(stripAnsi(chalkWrapped))).not.toContain(secret)
      expect(redactSensitiveData(stripAnsi(chalkWrapped))).toContain('sk_live_[REDACTED]')
    })
  })
})
