...
describe('runMigrationsSafe', () => {
  it('handles "duplicate column" SQLite error gracefully — does not throw', async () => {
  ...
  it('re-throws non-duplicate-column errors from migrations', async () => {
    if (!isBetterSqlite3Available()) {
      console.log('Skipping test: no SQLite driver available')
      return
    }
  ...
  it('handles FTS5 migration failure gracefully — does not throw', async () => {
    if (!isBetterSqlite3Available()) {
      console.log('Skipping test: no SQLite driver available')
      return
    }
  ...
  it('returns migration count run (0 when already up-to-date)', async () => {
    if (!isBetterSqlite3Available()) {
      console.log('Skipping test: no SQLite driver available')
      return
    }
  ...
  it('runs pending migrations and records version increments', async () => {
    if (!isBetterSqlite3Available()) {
      console.log('Skipping test: no SQLite driver available')
      return
    }
...
describe('closeDatabase', () => {
  it('closes an open database without error', async () => {
    if (!isBetterSqlite3Available()) {
      console.log('Skipping test: no SQLite driver available')
      return
    }
...
describe('createDatabaseAsync - schema initialization', () => {
  it('creates schema_version table and sets to current version', async () => {
    if (!isBetterSqlite3Available()) {
      console.log('Skipping test: no SQLite driver available')
      return
    }
  ...
  it('initializes all required tables including schema_version', async () => {
    if (!isBetterSqlite3Available()) {
      console.log('Skipping test: no SQLite driver available')
      return
    }
