...
describe('WEAK_PASSWORD_LEXICON_SOURCE provenance', () => {
  ...
  it('entries matches the emitted Set size, and is inside [2000, 6000] (M-2)', () => {
    ...
    //
    // SMI-6441 Wave 2, adversarial review round 2: 4,079 -> 4,012. Two
    // independent reviewers found the keeplist had no software-engineering
    // vocabulary section, so ordinary documentation nouns (`security`,
    // `command`, `cloud`, `scanner`, `matrix`, …) were still in the lexicon
    // and vetoed benign two-token labels to HIGH — which BLOCKS AN INSTALL,
    // since that gate fails on hasHigh alone. 67 tokens subtracted; this
    // literal moving is the intended, reviewed consequence, not drift.
