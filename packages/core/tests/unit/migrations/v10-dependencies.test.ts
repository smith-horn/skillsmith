...
describe("Migration v10: skill_dependencies table", () => {
  ...
  it("allows same skill_id + dep_target with different dep_source", () => {
    ...
    const count = db
      .prepare("SELECT COUNT(*) as cnt FROM skill_dependencies")
      .get() as { cnt: number };
  ...
  describe("CHECK constraints", () => {
    it("rejects invalid dep_type", () => {
      ...
      expect(() =>
        insert.run("author/skill-a", "invalid_type", "target", "declared"),
    ...
    it("rejects invalid dep_source", () => {
      ...
      expect(() =>
        insert.run("author/skill-a", "mcp_server", "target", "invalid_source"),
      ).toThrow(/CHECK constraint failed/);
    });
    ...
    it("rejects confidence outside 0.0-1.0 range", () => {
      ...
      expect(() =>
        insert.run("author/skill-a", "mcp_server", "target", "declared", 1.5),
      ).toThrow(/CHECK constraint failed/);

      expect(() =>
        insert.run("author/skill-b", "mcp_server", "target", "declared", -0.1),
      ).toThrow(/CHECK constraint failed/);
    });
