...
4. **Pre-push enforcement** (SMI-4764 Wave 0, `.husky/pre-push` Phase 6 → `scripts/eval-baseline-validator.mjs`): if the push touches a ranking file (`rerank.ts`, `search.ts`, `corpus.config.json`, anything under `eval/`), the validator looks up ALL `.signatures.log` lines matching the local `baseline.json` SHA-256 (there can be more than one — the same content can legitimately be re-signed more than once within the 15-line FIFO window) and, among those, checks whether at least one's recorded `headSha` (SMI-5708 Item #5) is the current git HEAD or an ancestor of it — not required to be an exact match, since new commits routinely land on top of the signed commit on the same branch (SMI-2597 wave-branch-stacking) and an exact-match requirement would otherwise false-reject a signature that's still legitimately fresh. (An actual `git rebase`/`commit --amend` produces a sibling commit, not a descendant, so it is correctly NOT tolerated — it requires a fresh signature.) Hash absent, or hash present but none of its matching entries' `headSha`s are HEAD or an ancestor of it (e.g. all recorded against an unrelated branch, or a commit never merged into this history), in **canonical mode** (`SKILLSMITH_EVAL_CANONICAL=true`) → push **rejected** with copy-pasteable repro. Same failure in **advisory mode** (default) → warning printed, push proceeds. At least one matching entry with an acceptable `headSha` → silent pass (subject to the freshness window below, checked against that same entry).

   **Honest scope**: this is a per-developer *accidental-staleness* check, not a security control. It catches "you forgot to re-run the eval before pushing" or "you pushed from a branch whose baseline.json was never actually validated here" — it does **not** and structurally **cannot** stop a determined bad actor, since `baseline.json` and `.signatures.log` are both ordinary tracked files: anyone with write access can hand-edit `baseline.json`, compute its SHA-256, and append a matching line (with any `headSha`, including a real current HEAD) in the same commit. Real provenance would require CI-side signing (an artifact signature tied to the evaluated commit + corpus hash + runner config hash, generated only by trusted CI) — tracked as a follow-up, not built in this pass. Don't read a pass here as more than "looks fresh."
...
6. **CI gate** (Retrieval Eval Gate in `.github/workflows/ci.yml`): on every PR that touches a ranking file, runs `check-baseline-drift.ts` against the hybrid threshold, posts a sticky comment with overall + per-category recall@5 deltas, and emits an advisory annotation (`audit:standards` check 45 — a second, independent `.signatures.log` consumer from the pre-push validator above) if the new `baseline.json` SHA-256 has no matching signature in `.signatures.log`, **or** none of its matching entries' `headSha` is the current HEAD or an ancestor of it (same ancestor-tolerant, multi-entry-aware rule as step 4, SMI-5708 Item #5(d)). Purely advisory — never fails CI, same honest-scope caveat as step 4 above.
