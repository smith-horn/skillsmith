...
`SessionStart` hooks fire on `source=startup` for branches containing an SMI/wave token anywhere in the name (`smi-NNN` or `wave-NNN`; covers `fix/smi-…`, `chore/smi-…`, etc — SMI-4809 broadened the matcher from literal-prefix only). Deny list: `main`, `hotfix-*`, `dependabot/*`, `renovate/*`, `release/*`, `revert/*`. Two hooks: priming (`scripts/session-start-priming.sh`, disable via `SKILLSMITH_DOC_RETRIEVAL_DISABLE_PRIMING=1`) + audit (SMI-4590 — Team/Enterprise namespace audit, 24h debounce, fail-soft, tier-gated; disable via `SKILLSMITH_SESSION_AUDIT_DISABLE=1`). A third surface, the **auto-heal consumer**, reads the `~/.skillsmith/retrieval-autoheal.state` file to surface a failing host native-binding auto-heal in the priming banner — it is a *consumer of state, not a hook* (SMI-5426; the heal itself is triggered by `.husky/post-merge`). Disable: `SKILLSMITH_RETRIEVAL_AUTOHEAL_DISABLE=1`. Full mechanism: [ruvector-dev-tooling.md § Session Priming](.claude/development/ruvector-dev-tooling.md#session-priming-smi-4451).
