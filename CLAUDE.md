...
**All secrets via Varlock. Never expose API keys in terminal output.** Commit `.env.schema` (defines `@sensitive`) and `.env.example` (placeholders); **never** `.env`. Run with secrets: `varlock run -- npm test`. Validate: `varlock load` (default pretty format — masked) or `varlock load --quiet` (validation only). **Never** `varlock load --format json` / `json-full` / `json-full-compact` / `env` — those emit **unmasked** values. **Never** read the contents of `.env`, `.env.registry`, or any `.worktrees/**/.env` via *any* command — `cat`, `grep`, `head`, `tail`, `sed`, `awk`, `less`, `strings`, `xxd`, an inline `python3 -c` / `node -e`, or a `docker exec skillsmith-dev-1 …` wrapper around any of them (the repo root is bind-mounted at `/app`, so `/app/.env` is the same file). **Never** `echo $SECRET`. The one sanctioned exception is an output-free presence check — `grep -qE '^KEY=' .env` (exit code only, no content). `.env.schema` and `.env.example` are always safe to read. Never ask users to paste secrets in chat. See [AI Agent Secret Handling](docs/internal/architecture/standards-security.md#411-ai-agent-secret-handling-smi-1956).
...
| Problem | Fix |
|---------|-----|
| A `grep`/`head`/`sed`/`cat`/inline-interpreter read of `.env` or `.env.registry` — or a `docker exec … /app/.env` wrapper around one — is denied with an `[env-read-guard]` reason | The `PreToolUse` Bash guard (`scripts/env-read-guard.mjs`) blocks any command that names a secret-bearing env file as a read target. It inspects the whole command string, so argument order and a `docker exec` wrapper do not evade it. Inspect config state with `varlock load` (default pretty format, masked) or `varlock load --quiet` — **not** `--format json`/`json-full`/`json-full-compact`/`env`, which are unmasked and also denied. `.env.schema` and `.env.example` stay readable; an output-free presence check (`grep -qE '^KEY=' .env`) is explicitly allowed. Genuine false positive: `SKILLSMITH_ENV_READ_GUARD_DISABLE=1` for that invocation, and file a Linear issue so the shape gets added to the allow list. Registered in [guards-and-opt-outs.md](docs/internal/process/guards-and-opt-outs.md). |
...
| `submodule-pointer-regression`-labeled GitHub Issue fired (SMI-6260) | `pointer-autorepair` (`.github/workflows/submodule-pointer-check.yml`) detected a `docs/internal` gitlink FAIL on a push to `main` that it could not safely auto-repair. The issue body names the exact rule that fired (R1 orphaned-never-pushed, R5 orphaned-tip, R6 diverged, R7 backward regression relative to what `main` already registered, R8 PAT-unavailable, or R11 already-registered pointer unresolvable) and — for R1/R5/R6/R7 — the exact `./scripts/bump-docs-pointer.sh` invocation that resolves it; for R11, a maintainer must manually repair `main`'s pointer directly (see [ADR-143](docs/internal/adr/143-docs-internal-gitlink-ancestry-invariant.md)'s `docs-internal-pointer-repair` note) before R7 can validate future bumps against it. Confirm the current state live: `git -C docs/internal fetch origin main --prune && git -C docs/internal merge-base --is-ancestor <registered-sha> origin/main && echo behind`. Note: an R3 (behind) detection does NOT open this issue while `SKILLSMITH_MAIN_PUSH_PAT` is provisioned and shadow mode is off — it auto-repairs silently instead; R3 only opens an issue when the auto-repair push was suppressed (shadow mode, unprovisioned PAT, or a push failure) — see [guards-and-opt-outs.md](docs/internal/process/guards-and-opt-outs.md) for the exact shadow/kill-switch var names. Dedup key is the exact commit SHA `pointer-autorepair` reacted to, so a fresh regression on a later push always gets its own issue even if an older one is still open. |
...
| A worktree's edits to `.husky/*` or `scripts/*` don't take effect on `git push`/`git commit`, though `bash scripts/<x>.sh` run directly does (SMI-6334) | `core.hooksPath` drifted to an absolute path into the MAIN checkout, so every worktree ran main's hooks and scripts. Verify with `git config --get core.hooksPath` — it must be the relative literal `.husky/_`, not an absolute path. Fix: `./scripts/repair-worktrees.sh`, or directly `git config core.hooksPath .husky/_` (one shared value; fixes main and every worktree at once). Disable the self-heal: `SKILLSMITH_HOOKS_PATH_HEAL_DISABLE=1`; disable the pre-push warning: `SKILLSMITH_SKIP_HOOKS_PATH_CHECK=1`. [Details](docs/internal/implementation/smi-6334-worktree-hookspath-fix.md) |
