...
| `prod-deploy-approval-cancelled` GitHub Issue fired | A `Deploy to Production` job in `website-deploy-staging.yml` was cancelled after reaching its approval gate or mid-deploy. Check whether it was an intentional manual cancellation (close the issue if so — this detector cannot distinguish intentional from accidental); otherwise confirm the live site matches the latest tested `main` commit and re-trigger the deploy if stale. If this recurs, verify `deploy-production`'s job-level concurrency group (`website-production-deploy-*`, `cancel-in-progress: false`) wasn't reverted. Disable: `SKILLSMITH_PROD_DEPLOY_CANCEL_MONITOR_DISABLE=1` (SMI-6208) |
