...
**Scope boundary — team resolution is not caller identity**: resolving a team answers *which team*, never *who is calling* (a team's license key is shared, and the RPC returns a team, never a person). `private_registry_*`'s `publish` / `install` / `submissions` / `approve` / `reject` / `deprecate` / `undeprecate` therefore additionally require the signed-in user's own Supabase JWT via `resolveUserAccessToken()` — i.e. `skillsmith login` on the machine. Neither env var removes that requirement.
