...
**Team-scoped tools** (`team_workspace`, `share_skill`, `private_registry_*`) additionally require a **team credential** to resolve the caller's team. Resolution path: team credential → SHA-256 → `license_keys.key_hash` → `subscriptions` → `teams.subscription_id`, via the `resolve_team_from_license` RPC (migration 071, SECURITY DEFINER, invoked via anon client). Missing/invalid keys return a typed error (not stub data) when Supabase is configured.

**`SUPABASE_SERVICE_ROLE_KEY` is NOT used by `team_workspace`/`share_skill` as of SMI-6113 + SMI-6241 Wave 2.** Before that fix, all 8 methods ran on the service-role client, which bypasses RLS entirely — any team member could create or delete a workspace, an action the product has always intended to be admin-only. They now run on the caller's own signed-in-user JWT via `team-workspace.live.auth.ts`'s two named getters (`getWorkspaceManageUserClient` for create/delete, `getWorkspaceMemberUserClient` for the other 6 methods), same pattern as `private_registry_*` below — see "Scope boundary" for why a team credential alone can never be enough. **Every one of the 8 methods now requires `skillsmith login` on the MCP host, including the previously-unauthenticated reads** (`list`/`get`/`listSkills`/`getWorkspaceSettings`) — a hard prerequisite change for any existing Team-tier install, not just the two admin-gated actions.

**Which env var supplies the team credential (SMI-6080)**: `readLicenseKey()` (`packages/mcp-server/src/tools/team-resolver.ts`) reads, in order, an explicit argument → `SKILLSMITH_LICENSE_KEY` → `SKILLSMITH_API_KEY`. The API-key fallback is not a broadened attack surface: `resolve_team_from_license(p_license_key TEXT)` SHA-256-hashes whatever string it is handed and matches `license_keys.key_hash`, so a plain `sk_live_*` personal API key resolves through the *identical* lookup a signed license blob does — same table, same hash, same team. It exists because an account whose Enterprise access came from `admin-grant-subscription` holds only a plain API key and never a signed JWT license blob, while the separate tier feature-gate (`middleware/license.ts`) requires `SKILLSMITH_LICENSE_KEY` to be *unset* so it can resolve tier live via `SKILLSMITH_API_KEY` → `/license-status`; before the fallback, no single env configuration satisfied both gates. `SKILLSMITH_LICENSE_KEY` still wins when both are set, so accounts holding a real license blob are unaffected.

**Scope boundary — team resolution is not caller identity**: resolving a team answers *which team*, never *who is calling* (a team's license key is shared, and the RPC returns a team, never a person). `private_registry_*`'s `publish` / `install` / `submissions` / `approve` / `reject` / `deprecate` / `undeprecate`, and (as of Wave 2) every `team_workspace`/`share_skill` method, therefore additionally require the signed-in user's own Supabase JWT via `resolveUserAccessToken()` — i.e. `skillsmith login` on the machine. Neither env var removes that requirement.
