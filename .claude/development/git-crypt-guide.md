...
**Pre-commit hook behaviour in worktrees (SMI-5570/SMI-5074, SMI-6568)**: `scripts/lib/hook-docker-detect.sh` routes on any OS. If this worktree's own container is running, pre-commit's type check runs in it (gitleaks and lint-staged always run on the host). If the container is down, the hook hard-fails with a `worktree-docker.sh start` command. It falls back to host instead when every staged file is docs, when nothing is staged, or when `SKILLSMITH_PRE_PUSH_HOST=1` is set, unless `SKILLSMITH_PRE_PUSH_DOCKER=1` is set, which always requires the container. Pre-commit classifies the staged set; pre-push classifies the commits being pushed. Off-tree worktrees always run on the host.
...
**Worktree own-container-down → host fallback (SMI-4377 + SMI-4381 + SMI-4549 + SMI-4681 + SMI-4686 + SMI-6568)**: detection logic lives in one place — `scripts/lib/hook-docker-detect.sh` — sourced by all six hook callers (`.husky/pre-commit`, `.husky/pre-push`, `scripts/pre-push-check.sh`, `scripts/pre-push-coverage-check.sh`, `scripts/lib/check-native-modules.sh`, `scripts/lib/check-container-deps-fresh.sh`), on any OS — this is not a macOS-specific code path (see the Background section of the lib itself: the routing this fixes is a generic Docker/Linux-kernel mount(2) behavior). When this worktree's own container isn't running, the hook either hard-fails with a `worktree-docker.sh start` command, or falls back to host with one of these messages:

```text
📂 Docs-only commit — falling back to host execution instead of hard-failing (worktree's own container '<container>' isn't running)
📂 No staged file changes — falling back to host execution instead of hard-failing (worktree's own container '<container>' isn't running)
📂 Docs-only push — falling back to host execution instead of hard-failing (worktree's own container '<container>' isn't running)
   Per-package node_modules symlinks are not traversable in
   Docker Desktop's virtiofs. Host resolution works correctly.
```

…it is **expected** and the hook is doing the right thing. Pre-commit classifies the **staged set**; pre-push classifies the **committed range** being pushed — same fallback mechanism, different subject (SMI-6568). If the message is followed by `❌ Host node_modules missing in worktree.`, run `./scripts/repair-worktrees.sh` to backfill the symlinks + native bindings.

**Escape hatch — `SKILLSMITH_PRE_PUSH_DOCKER=1` (SMI-4767, SMI-6568)**: the host fallback above is correct for docs-only/empty commits and pushes, but breaks `pre-push` test runs because the parent vitest invocation in `scripts/pre-push-coverage-check.sh` still inherits parent-worktree `GIT_*` env. SMI-4693 only landed Waves 1–3 (test-fixture env scrub via `git-fixture-env`); the parent-vitest leak is deferred to SMI-4769. This var is evaluated before any classification and without checking the caller (R2-F1), so it also applies to `pre-commit` — a docs-only or empty-staged commit still hard-fails if set and the worktree's own container is down. Workaround: prepend `SKILLSMITH_PRE_PUSH_DOCKER=1` to the push (or commit) to force Docker even on macOS worktrees, bypassing this fallback:
...
**Escape hatch — `SKILLSMITH_PRE_PUSH_HOST=1` (SMI-5570/SMI-5074, SMI-6568)**: falls back pre-commit and pre-push execution to the host — but ONLY when the worktree's own dedicated container isn't detected as running at all (`hook-docker-detect.sh`'s `DOCKER_AVAILABLE=0` gate, `scripts/lib/hook-docker-detect.sh:263`), and unless `SKILLSMITH_PRE_PUSH_DOCKER=1` is also set, which takes precedence and hard-fails instead (R2-F1). If the container IS up but internally unhealthy (e.g. the SMI-5635 read-only-mount native-module/symlink defect), this var is a no-op: `USE_DOCKER` stays 1 and every pre-push phase — security suite, format, coverage/tests — still runs in the broken container. There is no flag that forces host execution over an already-running-but-broken container; the only paths in that situation are fixing the container directly (this is a WORKTREE's own dedicated container — pre-commit and pre-push both route through it, not `skillsmith-dev-1` — so recover it with `( cd <this worktree's path> && docker compose --profile dev restart dev )`, which self-heals native bindings, SMI-5351; never run an npm-mutating command against a worktree container, CLAUDE.md) or `git commit`/`git push --no-verify`.

**Caveat (SMI-4698)**: the **native-rebuild step** of `./scripts/repair-worktrees.sh` aborts if a `skillsmith*-dev-N` container is running — it would otherwise overwrite the container's ELF native bindings via the symlinked `node_modules`. Symlink-repair steps run safely regardless. Stop the container first (`docker compose --profile dev down`), or pass `--force-with-active-docker` and run `docker exec -w /app skillsmith-dev-1 sh -c 'sh scripts/lib/node-modules-mount-gate.sh && npm rebuild better-sqlite3 onnxruntime-node'` afterward to restore the container's bindings (SMI-6614: a non-zero exit with no rebuild output means at least one node_modules path isn't mounted with a volume-shaped root, recreate first). `repair-worktrees.sh` itself (round-4) prints this exact form only for `skillsmith-dev-1` — a WORKTREE's own container instead gets the `docker compose --profile dev restart dev` recovery, since an npm-mutating command must never run against a worktree container.
