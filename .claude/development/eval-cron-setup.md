...
**Invariant: no self-silencing TTL.** The script deliberately does **not** apply the interactive `probe.ts` function's `OUTAGE_MARKER_TTL_DAYS = 7` self-silencing — a present outage marker always alerts. The interactive banner stops nagging after 7 days; the _scheduled backstop's_ whole purpose is the opposite — keep alerting until the binding actually heals (self-detected on the next successful row write).
