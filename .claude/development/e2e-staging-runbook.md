...
**Wave 3 Phase A verification (2026-08-10, PR #2262, job `93510589769`) — fix confirmed live:** with secrets still absent, this same `pull_request`-triggered run of the fixed workflow correctly reported the spec as failed: the guard emitted `::warning::spec exit_code='2' outcome=failure...`, the step summary read "spec FAILED (exit_code='2', outcome=failure)", the artifact `private-registry-e2e-31405451324` uploaded containing exactly `[e2e-registry] Missing required env var: E2E_REG_USER_PASSWORD`, and the job/workflow conclusion stayed green (masked, by Phase-1 design) while the spec step itself genuinely failed. **REST API `conclusion`-field mapping, empirically pinned**: `gh api repos/.../actions/jobs/<id>` reports the spec step's `conclusion` as `"success"` even on this real failure — that field reflects the *post*-`continue-on-error` value only, never the raw pre-mask result, regardless of whether `continue-on-error` is at job or step level. The only place the raw result is visible is inside the workflow engine's own expression context (`steps.spec.outcome`, readable in `if:` conditions and step `env:` blocks) — proven here by the "Upload artifacts on spec failure" step actually running (not skipped) under its `if: steps.spec.outcome == 'failure'` condition. **Do not use the REST `conclusion` field to check this gate from outside the workflow** — use the guard's step-summary line, its `::warning::` annotation, or the uploaded artifact instead.
