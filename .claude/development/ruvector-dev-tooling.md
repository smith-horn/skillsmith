...
A `SessionStart` hook (`scripts/session-start-priming.sh`) writes a transient priming index to `/tmp/session-priming-${SESSION_ID}.md` and pipes the same content into initial context as `additionalContext`. Fires only on `source=startup` and branches containing an SMI/wave token (`smi-NNN` or `wave-NNN`) anywhere in the name — covers `smi-NNN-foo`, `fix/smi-NNN-foo`, `chore/smi-NNN`, `feat/wave-N-bar`, etc. (SMI-4809 broadened the matcher; before it, only literal-prefix `smi-*`/`wave-*` primed). Deny list (no priming) covers `main`, `hotfix-*`/`hotfix/*`, `dependabot/*`, `renovate/*`, `release/*`, `revert/*`, and the empty branch (detached HEAD) regardless of any embedded token. Otherwise no-op. The transient file is mode 0600 and swept after 24h. Disable with `SKILLSMITH_DOC_RETRIEVAL_DISABLE_PRIMING=1`. The underlying retrieval index lives in `packages/doc-retrieval-mcp/`.
